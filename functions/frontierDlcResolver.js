...
// Frontier stores the used DLC set in nRequiredDLC and exposes additional
// content/filter identifiers in some creation types. Keep this server mapping
// aligned with the desktop resolver so verified documents contain display
// names while retaining every raw/unknown bit.
...
function normalizeFrontierDlcCatalog(gameId, rawCatalog) {
    ...
    const saveMappings = rawCatalog?.saveMappings &&
        typeof rawCatalog.saveMappings === "object" ? rawCatalog.saveMappings : {};
    const entries = [];
    ...
    for (const rawName of names) {
        ...
        const bit = Number.isSafeInteger(configured?.bit) &&
            configured.bit >= 0 && configured.bit <= 52 ? configured.bit : fallback?.bit;
        const configuredIdentifiers = Array.isArray(configured?.identifiers) ?
    ...
    return {
        gameId,
        version: Number.isSafeInteger(rawCatalog?.catalogVersion) ?
            rawCatalog.catalogVersion : 2,
...
function getCatalog(gameId, catalog) {
    ...
    const configuredEntries = catalog.entries
        ...
        .map((entry) => ({
            ...
            bit: Number.isSafeInteger(entry.bit) && entry.bit >= 0 && entry.bit <= 52 ?
                entry.bit : null,
    ...
    const configuredByName = new Map(configuredEntries
...
function resolveFrontierDlcRequirements(
    gameId,
    rawMask,
    rawIdentifiers = [],
    ...
    identifiers.forEach((identifier) =>
        addName(byIdentifier.get(identifier.toLowerCase())));
    return {
        mappingVersion: catalog.version,
        requiredDlcs,
        requiredDlcBits,
        unknownDlcBits: requiredDlcBits
            .filter((bit) => !byBit.has(bit)),
