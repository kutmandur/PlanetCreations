...
import {
    ...
    normalizePersonalTheme, readCachedPersonalTheme, readableThemeText, resolvePersonalThemeColors, validatePersonalTheme,
} from '../../utils/personalTheme';
import InfoBox from './InfoBox';
import { extractThemePalette } from '../../utils/themePalette';

const sameTheme = (a, b) => ['backgroundUrl', 'cardColor', 'accentColor', 'adaptToColorScheme'].every(key => a[key] === b[key]);
...
export default function ThemeSettings({ user }) {
    ...
    const [previewUrl, setPreviewUrl] = useState(form.saved.backgroundUrl);
    ...
    useEffect(() => {
        if (!dirty) { setPreviewUrl(saved.backgroundUrl); setImageError(false); }
    }, [saved.backgroundUrl, dirty]);
    ...
    const showPreview = () => {
        setPreviewUrl(normalized.backgroundUrl);
        setImageError(false);
    };
    const suggestColors = async () => {
        if (!normalized.backgroundUrl || analysisRef.current) return;
        const controller = new AbortController();
        analysisRef.current = controller;
        setAnalyzing(true);
        setPaletteError('');
        try {
            const result = await extractThemePalette(normalized.backgroundUrl, { signal: controller.signal });
    ...
    const handleSave = async event => {
        ...
        try {
            ...
            setPreviewUrl(theme.backgroundUrl);
            setImageError(false);
        ...
        <form onSubmit={handleSave}>
            <fieldset disabled={saving} className="min-w-0 space-y-5">
                <div>
                    ...
                    <p id="theme-background-help" className="text-xs text-gray-500 mt-2">Paste a Postimages Direct Link. Leave empty to use the default background.</p>
                    <InfoBox />
                </div>
                ...
                <div className="rounded-lg border p-4 space-y-3">
                    <div className="flex flex-wrap items-center justify-between gap-3">
                        ...
                        <button type="button" onClick={suggestColors} disabled={!normalized.backgroundUrl || analyzing || Boolean(palette)}
                ...
                <div>
                    ...
                    <div className={draft.adaptToColorScheme ? 'grid gap-3 sm:grid-cols-2' : ''}>
                        {(draft.adaptToColorScheme ? ['light', 'dark'] : ['current']).map(mode => {
                            ...
                            return <div key={mode} role="group" aria-label={mode === 'current' ? 'Theme preview' : `${mode === 'dark' ? 'Dark' : 'Light'} mode preview`}
                                className="pc-theme-preview relative isolate overflow-hidden rounded-xl bg-gray-100 dark:bg-gray-900 p-4 sm:p-6"
                                style={mode === 'current' ? undefined : { backgroundColor: mode === 'dark' ? '#111827' : '#f3f4f6' }}>
                                {previewUrl && !imageError && <img key={previewUrl} src={previewUrl} alt="" referrerPolicy="no-referrer"
                                    onError={() => setImageError(true)} className="absolute inset-0 -z-10 h-full w-full object-cover" />}
                    ...
                    {imageError && <p className="text-sm mt-2" role="status">The image could not be loaded. Check the Direct Link or try the preview again. The default background will be used if the image is unavailable.</p>}
                </div>
                ...
                <div className="flex flex-wrap items-center gap-3">
                    ...
                    <button type="button" onClick={() => { setForm(previous => ({ ...previous, draft: { ...DEFAULT_PERSONAL_THEME } })); setPreviewUrl(''); setMessage('Defaults selected. Save theme to apply.'); setSaveError(''); }}
