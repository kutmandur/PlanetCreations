...
import { doc, getDoc, writeBatch, collection, serverTimestamp, query, where, getDocs } from 'firebase/firestore';
import Spinner from '../ui/Spinner';
import Icon from '../ui/Icon';
import { ICONS, containsBlacklistedWord } from '../../utils/helpers';
import InfoBox from '../ui/InfoBox';

const CreateCommunityForm = ({ user, setModalMessage, blacklist }) => {
    ...
    const fetchDiscordRanks = async (isSilent = false) => {
        ...
        try {
            const functionUrl = "https://us-central1-planetcreationsdotnet.cloudfunctions.net/api/getDiscordRoles"; 
            const response = await fetch(`${functionUrl}?serverId=${discordServerId}`);
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!user) {
            setModalMessage("You must be logged in to create a community.");
            return;
        }
        if (!name.trim() || !description.trim()) {
            setModalMessage("Community Name and Description are required.");
            return;
        }
        ...
        const communitySlug = slugify(name);
        const slugQuery = query(collection(db, 'communitys'), where("slug", "==", communitySlug));
        const slugSnapshot = await getDocs(slugQuery);
        if (!slugSnapshot.empty) {
            setModalMessage("A community with this name already exists. Please choose another name.");
            setLoading(false);
            return;
        }

        if (discordServerId) {
            const q = query(collection(db, 'communitys'), where("discordServerId", "==", discordServerId));
        ...
        try {
            ...
            if (!profileSnap.exists() || !profileSnap.data().username) {
                setModalMessage("Could not find your username. Please ensure your profile is set up correctly.");
                setLoading(false);
                return;
            }
            const ownerUsername = profileSnap.data().username;
            ...
            const communityRef = doc(collection(db, 'communitys'));
            ...
            const memberRef = doc(db, 'communitys', communityRef.id, 'members', user.uid);
            batch.set(memberRef, {
                roles: ['owner'],
            ...
            const userMembershipRef = doc(db, 'profiles', user.uid, 'communityMemberships', communityRef.id);
    ...
    return (
        <div className="max-w-4xl mx-auto mt-10 p-8 bg-white rounded-lg shadow-lg">
            <h1 className="text-3xl font-bold text-center mb-6">Create Community</h1>
            <form onSubmit={handleSubmit} className="space-y-8">
                <div>
                    <label className="block text-gray-700 font-bold mb-2">Community Name</label>
                    <input type="text" value={name} onChange={(e) => setName(e.target.value)} className="w-full p-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" required />
                </div>
                <div>
                    <label className="block text-gray-700 font-bold mb-2">Description</label>
                    <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows="4" className="w-full p-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" required></textarea>
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="url" value={profileImageUrl} onChange={(e) => setProfileImageUrl(e.target.value)} className="w-full p-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="https://..." />
                    <div className="mt-2"><InfoBox /></div>
                </div>
                <div>
                    <label className="block text-gray-700 font-bold mb-2">Community Theme Color</label>
                    <div className="flex items-center space-x-4">
                        <input type="color" value={themeColor} onChange={(e) => setThemeColor(e.target.value)} className="h-12 w-24 p-1 border rounded-lg cursor-pointer" />
                        <div className="p-3 rounded-lg font-mono text-lg text-white" style={{ backgroundColor: themeColor }}>{themeColor.toUpperCase()}</div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                        <a href="https://discord.com/oauth2/authorize?client_id=1407474623511269427&permissions=268435456&integration_type=0&scope=bot" target="_blank" rel="noopener noreferrer" className="bg-indigo-500 hover:bg-indigo-600 text-white font-bold py-2 px-4 rounded-lg text-center">
                            Invite Bot
                        </a>
                        <button 
                            type="button"
                            onClick={() => setIsServerIdInputVisible(!isServerIdInputVisible)}
                            className={`${discordServerId ? 'bg-green-500 hover:bg-green-600' : 'bg-indigo-500 hover:bg-indigo-600'} text-white font-bold py-2 px-4 rounded-lg`}
                        >
                        ...
                        >
                            {isFetchingRanks ? <Spinner size="small" /> : 'Import Ranks'}
                        </button>
                    </div>
                        ...
                        <div className="mt-4">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {suggestedRanks.map(rank => {
                                    const textColor = getTextColorForBackground(rank.color);
                                    return (
                                        <button 
                                            key={rank.id}
                                            type="button"
                ...
                <div className="flex justify-end mt-8">
                    <button type="submit" disabled={loading} className="bg-blue-500 hover:bg-blue-600 text-white font-bold py-3 px-6 rounded-lg disabled:opacity-50 transition-colors">
