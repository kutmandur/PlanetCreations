...
function readTrackedRideTestDataLayout(body, trackedRideCount) {
    ...
    if (!header || header.count !== trackedRideCount || header.count > MAX_RIDES) return null;
...
function parseTrackedRideTestDataCache(body, trackedRideCount) {
    ...
    if (!layout) return [];
    const { cacheEntries, pathLengths, totalSamples, dataOffset, blockStride } = layout;
    if (totalSamples === 0) {
        return cacheEntries.map(entry => ({ entityId: entry.entityId, stats: null }));
    }
    ...
    const readPathField = (pathIndex, fieldIndex) => {
        ...
        const fieldStart = dataOffset + fieldIndex * blockStride + 4;
...
function scanStringReferences(buffer, strings) {
    ...
    for (let offset = 0; offset < buffer.length; offset += 1) {
        ...
        if (value) references.push({ offset, endOffset: reference.offset, index: reference.value, value });
    }
    return references;
}

function zooTransportRide(typeId, name = null) {
    const category = humanizeRideType(String(typeId || '').replace(/^Transport_/i, ''), 'Transport ride');
    return {
        kind: 'tracked',
        name: typeof name === 'string' && name.trim() ? name.trim().slice(0, 500) : null,
        typeId: typeof typeId === 'string' && typeId ? typeId.slice(0, 500) : null,
...
function parsePlanetZooTransportRides(body, strings) {
    ...
    const references = scanStringReferences(body.subarray(header.dataOffset), strings);
    return references
        .filter(reference => /^Transport_[A-Za-z0-9_]+$/.test(reference.value))
        ...
        .map(transportReference => {
            const nameReference = references.find(reference =>
                ...
                !/^(?:Transport_|station_|train_default$|Scenario\d+_)/i.test(reference.value));
            return zooTransportRide(transportReference.value, nameReference?.value);
        });
}
...
function parsePlanetZooSaveMetadata(payload, kind) {
    ...
    const parsedRides = (kind === 'park' || kind === 'autosave') && clients.has('Track') ?
    ...
    return {
        ...
        rides: fillMissingRides(parsedRides, 'tracked', trackedRideCount, []),
    };
}
...
function parseCobraSaveMetadata(payload, kind, outerRatings = null, tags = [], options = {}) {
    ...
    const testData = testCacheBody ?
        parseTrackedRideTestDataCache(testCacheBody, trackedRideCount) : [];
